<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        function cadastrar($conexao, $tabela, $url, $admin, $user, $cpfLimpo, $email, $passCriptografada, $telefone){
            $cadastrar = $conexao->prepare("INSERT INTO $tabela (nome_vendedor, nome_url, cpf, admin, email, senha, telefone, foto) VALUES(?, ?, '$admin', ?, ?, ?, 'http://localhost/marketplace/fotosUsuarios/imgPadrao.png')");
            $cadastrar->bind_param('ssssss', $user, $url, $cpfLimpo, $email, $passCriptografada, $telefone);
        ...
        if(empty($_POST['user']) || empty($_POST['email']) || empty($_POST['cpf']) || empty($_POST['pass'])){
            ...
            if($resultado1->num_rows > 0 || $resultado2->num_rows > 0){
                ...
                if(filter_var($email, FILTER_VALIDATE_EMAIL)){
                    ob_clean();
                    echo cadastrar($conexao, $tabela, $userUrl, $admin, $user, $cpfLimpo, $email, $passCriptografada, $telefone);
